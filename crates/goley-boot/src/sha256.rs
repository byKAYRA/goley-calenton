...
pub(crate) fn digest(bytes: &[u8]) -> [u8; 32] {
    ...
    let mut exact_chunks = bytes.chunks_exact(64);
    for chunk in &mut exact_chunks {
        compress(
            &mut state,
            chunk.try_into().expect("SHA-256 block has fixed size"),
        );
    }
...
fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    for (index, bytes) in block.chunks_exact(4).enumerate() {
        words[index] = u32::from_be_bytes(bytes.try_into().expect("word has fixed size"));
    }
    for index in 16..64 {
...
mod tests {
    ...
    #[test]
    fn matches_standard_empty_vector() {
        assert_eq!(
            digest_hex(b""),
    ...
    #[test]
    fn matches_standard_abc_vector() {
        assert_eq!(
            digest_hex(b"abc"),
